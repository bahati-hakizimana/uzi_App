...
const Quizes = () => {
  ...
  const fetchQuizes = async (page, token) => {
    try {
      const response = await fetch(`https://api.uzi.ishemahub.com/api/v2/quiz?pageNumber=${currentPage}&pageSize=3`, {
        method: 'GET',
        headers: {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this quiz?')) {
      try {
        const response = await fetch(`https://api.uzi.ishemahub.com/api/v2/quiz/${id}`, {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        if (response.ok) {
          ...
          alert('Failed to delete Quiz:', response.statusText);
        }
      } catch (error) {
          ...
          <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    <th scope="col" class="px-4 py-3">Quiz_Id</th>
                    <th scope="col" class="px-4 py-3">Level_Id</th>
                    <th scope="col" class="px-4 py-3">Questions</th>
                    <th scope="col" class="px-4 py-3">User_id</th>
                    <th scope="col" class="px-4 py-3">Score</th>
                    <th scope="col" class="px-4 py-3">OutOf</th>
                    <th scope="col" class="px-4 py-3">Cateegory_id</th>
                ...
                <tbody>
                  {filteredQuizes.map((quiz) => (
                    <tr class="border-b dark:border-gray-700">
                      ...
                      <td class="px-4 py-3">{quiz.level.id}</td>
                      <td class="px-4 py-3">{quiz.questions}</td>
                      <td class="px-4 py-3">{quiz.user.id}</td>
                      <td class="px-4 py-3">{quiz.score}</td>
                      <td class="px-4 py-3">{quiz.outOf}</td> 
                      <td class="px-4 py-3">{quiz.category.id}</td>
                      
                      <td class="px-4 py-3">{quiz.created_at}</td>
                      <td class="px-4 py-3">{quiz.updated_at}</td>
                      <td class="px-4 py-3 flex items-center justify-end">
                        <button onClick={() => handleDelete(quiz.id)} class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                          <MdDeleteForever size={25} className='text-purple-400' />
                        </button>
                        <button onClick={() => handleEdit(quiz.id)} class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
              <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                Showing
                <span class="font-semibold text-gray-900 dark:text-white">{filteredQuizes.length > 0 ? (currentPage - 1) * 3 + 1 : 0}-{(currentPage - 1) * 3 + filteredQuizes.length}</span>
                of

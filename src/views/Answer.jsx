...
function Answer() {
  ...
  useEffect(() => {
    ...
    if (storedToken) {
      ...
      fetchAnswers(1, storedToken);
    }
  }, []);

  useEffect(() => {
    if (token) {
      fetchAnswers(currentPage, token);
    }
  }, [currentPage, token]);
  const fetchAnswers = async (page, token) => {
    try {
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  <th scope="col" class="px-4 py-3">Answer_Id</th>
                  <th scope="col" class="px-4 py-3">Title</th>
                  <th scope="col" class="px-4 py-3">Questions_id</th>
              ...
              <tbody>
                {filteredAnswers.map((answer) => (
                  <tr class="border-b dark:border-gray-700">
                    <td class="px-4 py-3">{answer.id}</td>
                    <td class="px-4 py-3">{answer.title}</td>
                    <td class="px-4 py-3">{answer.question.id}</td>
                    <td class="px-4 py-3">{answer.created_at}</td>
                    <td class="px-4 py-3">{answer.updated_at}</td>
                    <td class="px-4 py-3 flex items-center justify-end">
                      <button onClick={() => handleDelete(answer.id)} class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                        <MdDeleteForever size={25} className='text-purple-400' />
                      </button>
                      <button onClick={() => handleEdit(answer.id)} class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
          ...
          <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
            <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
              Showing
              <span class="font-semibold text-gray-900 dark:text-white">{filteredAnswers.length > 0 ? (currentPage - 1) * 3 + 1 : 0}-{(currentPage - 1) * 3 + filteredAnswers.length}</span>
              of
